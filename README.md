...
2. In the Supabase dashboard, go to **SQL Editor** and run the migration files in this exact
   ...
   - `supabase/migrations/0007_storage_policies.sql`
   - `supabase/migrations/0008_gcash_qr.sql`
...
5. Go to **Storage** and create a public bucket named `product-images` — this is required for the
   in-app "Upload from computer" button on the product edit form to work. Do this *before*
   running `0007_storage_policies.sql`, since that migration adds permission policies scoped to
   a bucket with that exact name (it won't error if the bucket doesn't exist yet, but uploads
...
1. **`seed.sql` was never run**, or was run *before* all the migration files — the two must be
   ...
   `0007_storage_policies.sql` → `0008_gcash_qr.sql` → then `seed/seed.sql`.
...
4. **If the error is specifically "infinite recursion detected in policy for relation
   'admins'"** — this was a real bug, fixed in `0006_fix_admin_rls_recursion.sql`. If you already
   ran `0001`–`0005` before this fix existed, just run `0006` now.

## Troubleshooting: admin saves fail with "HTTP 500" or similar

Every admin save now shows the real error message and status code when it fails (instead of a
vague "try again"). If you see a 500 with a message mentioning something like "service role" or
"Supabase service role client requires...", it means `SUPABASE_SERVICE_ROLE_KEY` (or
`NEXT_PUBLIC_SUPABASE_URL`) is missing or wrong in `.env.local` — double check it's the **service
role** key from Supabase's Settings → API page (not the anon key — they're easy to mix up), and
restart `npm run dev` after fixing it, since Next.js only reads `.env.local` at startup. For any
other 500, the message shown (and your terminal running `npm run dev`, which always logs the full
error) will say exactly what Supabase rejected.

**"Invalid settings data. (400)" specifically** was a real bug (now fixed) — the settings form
always saves its entire state, and the seed data's `logo_url` is a relative path
(`/images/brand/logo.jpg`) rather than a full URL, which an overly strict validation rule used to
reject outright. This blocked *every* settings save, not just image fields — if "Scheduled
Orders" or another toggle looked stuck even after you flipped it, this was almost certainly why.
If you're on an older copy of this project and still see it, update to the latest files.

## Every migration and the seed file are safe to re-run, always

As of this version, every file in `supabase/migrations/` and `supabase/seed/seed.sql` is fully
idempotent — every `CREATE TABLE`/`TYPE`/`POLICY`/`TRIGGER`/`CONSTRAINT` is guarded (`IF NOT
EXISTS`, or a `DROP ... IF EXISTS` right before it), and every `INSERT` uses `ON CONFLICT ... DO
UPDATE`. This was verified directly: the full sequence (all 7 migrations, then the seed file) was
run twice in a row against a real Postgres database with zero errors both times, producing
identical results.

**In practice, this means: if you're ever unsure what you've already run, it's always safe to
just re-paste the entire `0001` through `0007` sequence and then `seed.sql` again, in order,
regardless of what's already applied.** You do not need to figure out which ones you've already
run, and you should not truncate/drop tables to "start clean" — that only risks losing real data
(admin edits, orders, uploaded photos) for no benefit, since re-running is always safe instead.

If you ever see an "already exists" error from a migration file despite this, it means you're
running an older copy of that file — re-download the latest project files first.
