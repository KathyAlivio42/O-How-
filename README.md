...
1. **`seed.sql` was never run**, or was run *before* all the migration files — the two must be
   ...
   `0004_must_try_tag.sql` → `0005_seed_idempotency.sql` → then `seed/seed.sql`. If a migration
   is skipped, `seed.sql` references columns that don't exist yet (e.g. `is_must_try`), the whole
   script fails, and the Supabase SQL editor rolls back everything in that paste — including the
   category rows that looked like they should have been created first.
