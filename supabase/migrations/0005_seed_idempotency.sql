...
--
-- SAFE TO RE-RUN — see the header note in 0001_init.sql. (This DELETE is
-- itself naturally idempotent: once duplicates are gone, re-running it
...
do $$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'product_options_group_name_unique'
  ) then
