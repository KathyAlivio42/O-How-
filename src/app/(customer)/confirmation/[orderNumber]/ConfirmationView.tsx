...
export function ConfirmationView() {
  ...
  return (
    <main className="max-w-md mx-auto px-4 py-16 flex flex-col items-center text-center">
      ...
      {order.payment_method === "gcash" && order.payment_status === "pending" ? (
        <p className="text-sm text-clay mt-4">
          We're confirming your GCash payment. We'll start on your order as soon as it's
          verified.
        </p>
      ) : (
        <p className="text-sm text-ink-muted mt-4">
          {order.pickup_time_type === "scheduled" && order.scheduled_pickup_at
            ? `Scheduled for pickup at ${formatScheduledTime(order.scheduled_pickup_at)}`
