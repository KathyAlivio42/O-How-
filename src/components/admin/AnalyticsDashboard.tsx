...
import { getManilaParts, formatManilaMonthYear } from "@/lib/manila-time";
...
export function AnalyticsDashboard() {
  ...
  const dailySales = useMemo(() => {
    ...
    orders.forEach((o) => {
      // Group by the Manila calendar date the order was actually placed on,
      // not the UTC date embedded in the raw timestamp string -- an order
      // placed at 1:00 AM Manila is still the previous UTC calendar day.
      const { month, day: d } = getManilaParts(new Date(o.created_at));
      const dayKey = `${String(month).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
      const cur = byDay.get(dayKey) ?? { sales: 0, orders: 0 };
  ...
  return (
    <div className="px-5 pt-6 pb-10 max-w-5xl">
      <h1 className="font-headline text-2xl font-bold text-brand-dark mb-1">
        {formatManilaMonthYear(new Date())}
      </h1>
