...
// Returns the failure reason (or null on success). Callers must check this
// before updating local state or telling the user it worked -- silently
// assuming success on a failed request is what caused edits to "revert
// after refresh": the UI looked updated, but nothing was written to the
// database.
async function patchProduct(id: string, data: Record<string, unknown>): Promise<string | null> {
  try {
...
function ProductRow({
  ...
  async function toggle(field: "is_bestseller" | "is_active" | "is_featured" | "is_must_try") {
    ...
    setToggleError(null);
    const err = await patchProduct(product.id, { [field]: next });
    if (!err) {
  ...
  async function saveDetails() {
    ...
    const err = await patchProduct(product.id, patch);
    setSaving(false);
    if (!err) {
      onChange(product.id, patch);
      setExpanded(false);
    } else {
      setSaveError(err);
    }
        ...
        <div className="mt-3 pt-3 border-t border-ink/10 flex flex-col gap-2">
          ...
          {form.image_url && (
            <p className="text-xs text-ink-muted -mt-1">
              Photo uploaded -- click <strong>Save Changes</strong> below to apply it.
            </p>
          )}
